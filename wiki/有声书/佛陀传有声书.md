---
title: "《佛陀传》有声书"
published: true
tags: 佛陀传 故道白云 有声书
categories: 传记 有声书
---
# {{ page.title }}
演播：胡克  
翻译：何蕙仪  
作者：一行禅师  
一行禅师沉稳迷人地讲述了佛陀从出生、成长、出家、修行、得道、弘法，直至涅槃的完整一生。  
胡克的演播，更为之增色。  
来源：昌列寺网站。我对昌列寺一无所知。很感恩能从这里找到这部有声书。  
原站链接：[《佛陀传》有声书](http://www.changleisi.com/index/Audio/details.html?id=78)  

直接浏览：  
<!-- 第一部分：定义剧集基本信息和单集数据 -->
<div id="series-info" style="display: none;">
{
  "zhSeries": "佛陀传",
  "enSeries": "FoTuoZhuan", 
  "ext": "mp3",
  "episodeDigits": 3,
  "startEpisode": 1,
  "endEpisode": 104
}
</div>

<div id="episodes-data" style="display: none;">
[
  "作者序：佛教在人间",
  "译者序：你可与佛陀近距离接触",
  "牧童跟随佛陀踏上修行之路（上）",
  "牧童跟随佛陀踏上修行之路（下）",
  "佛陀讲《看顾水牛经》",
  "当牧童初次遇上佛陀",
  "受伤的天鹅",
  "饿晕后，佛陀放弃了苦行",
  "当年佛陀出生时",
  "少年时的佛陀",
  "遇见未来的妻子－耶输陀罗",
  "佛陀结婚了",
  "生老病死，一生都要面对",
  "佛陀有了自己的儿子",
  "为了寻找大道，只能不告而别",
  "开始独自修行（上）",
  "开始独自修行（下）",
  "生命无常，疾病，死亡总让人措手不及，追悔莫及",
  "放弃苦修，放弃避世，回归自己（上）",
  "放弃苦修，放弃避世，回归自己（下）",
  "那一晚，耶输陀罗睡着了吗",
  "佛陀彻底开悟了",
  "苦痛消失了，一切都不同了",
  "对橘子的专注，是一种修行",
  "你和我，或许曾存在于一个生命体之中",
  "不同的人，需要不同的法门",
  "初转法轮",
  "痛苦，是因为在用错误的方式生活",
  "皈依佛，皈依法，皈依僧",
  "回归当下这一刻，你才会觉醒",
  "万法因缘生，万法因缘灭（上）",
  "万法因缘生，万法因缘灭（下）",
  "世法燃烧",
  "佛陀僧团",
  "十大弟子舍利弗，目犍连",
  "五戒：不杀生，不偷盗，不邪淫，不妄语，不饮酒（上）",
  "五戒：不杀生，不偷盗，不邪淫，不妄语，不饮酒（下）",
  "我会在春天回去",
  "谣言，迟早会自动止息",
  "唯一不会退灭的美，是慈悲与解脱",
  "重聚：佛陀回家了",
  "佛陀说教四圣谛：苦集灭道",
  "有了解和爱，一切都能成就",
  "我的家庭是众生，我的家乡是大地，我的身份是僧人",
  "不同的阶级，皆可加入僧团",
  "佛陀，是觉悟者的意思",
  "以黄金铺满地",
  "爱里生苦",
  "慈悲之爱，是没有执着与分别心",
  "每个人的眼泪都是咸的",
  "父王安详离世（上）",
  "父王安详离世（下）",
  "女子也可受戒出家",
  "僧团分裂了",
  "导致平和，喜悦和解脱，才是正法",
  "制定七灭诤，调停内部纠纷",
  "向大地学习，向水学习（上）",
  "向大地学习，向水学习（下）",
  "因为僧众们不停地犯错，才有了戒律",
  "放开心怀，保护所有众生（上）",
  "放开心怀，保护所有众生（下）",
  "功德田（上）",
  "功德田（下）",
  "终止内心的暴力（上）",
  "终止内心的暴力（下）",
  "佛陀遭到诬陷",
  "每个人都有觉醒的潜能（上）",
  "每个人都有觉醒的潜能（下）",
  "观呼吸",
  "木筏不是彼岸",
  "一把珍贵的泥土",
  "听到讥讽批评不愤怒，听到称赞欣赏不享受",
  "一个人牵挂越多，痛苦就越多",
  "狮子吼",
  "舍利弗之吼",
  "过着虚伪的生活，是由内而外的腐烂",
  "生死轮转，不要害怕（上）",
  "生死轮转，不要害怕（下）",
  "空的碗里，满是空气（上）",
  "空的碗里，满是空气（下）",
  "生老病死，犹如四座山",
  "佛法如大海",
  "三法印，三解脱门",
  "佛陀死后会到哪里去",
  "超越这世间的生死（上）",
  "超越这世间的生死（下）",
  "僧团彻底分裂了",
  "默默反抗",
  "有人要暗杀佛陀",
  "三百多名离开的比丘，又回来了",
  "真正的快乐，来源于自在与自由（上）",
  "真正的快乐，来源于自在与自由（下）",
  "佛陀也会变老（上）",
  "佛陀也会变老（中）",
  "佛陀也会变老（下）",
  "生死如幻象，不要停下脚步",
  "过去现在未来，没有人可以超越佛陀的智慧（上）",
  "过去现在未来，没有人可以超越佛陀的智慧（下）",
  "别再依赖他人和他物（上）",
  "别再依赖他人和他物（下）",
  "佛陀入灭",
  "故道白云佛陀见过的白云，仍在天空，佛陀走过的路，仍在我们的脚下（上）",
  "故道白云佛陀见过的白云，仍在天空，佛陀走过的路，仍在我们的脚下（下）",
  "《佛陀传》胡克结束感受"
]
</div>

<!-- 简单样式 -->
<style>
.media-table {
  width: 100%;
  border-collapse: collapse;
  margin: 20px 0;
}
.media-table th, .media-table td {
  border: 1px solid #ddd;
  padding: 8px 12px;
  text-align: left;
}
.media-table th {
  background-color: #f2f2f2;
}
.download-btn, .play-btn {
  display: inline-block;
  padding: 4px 12px;
  margin: 2px;
  background: #28a745;
  color: white;
  text-decoration: none;
  border-radius: 4px;
  font-size: 14px;
  cursor: pointer;
  border: none;
}
.play-btn {
  background: #007bff;
}
.audio-player {
  width: 100%;
  margin: 10px 0;
  display: none;
}
</style>

<!-- 音视频列表将在这里生成 -->
<div id="media-list"></div>

<!-- 音频播放器容器 -->
<div id="audio-player-container" style="display: none;">
  <h3>正在播放：<span id="now-playing-title"></span></h3>
  <audio id="global-audio-player" class="audio-player" controls>
    您的浏览器不支持 audio 元素。
  </audio>
</div>

<!-- 第二部分：实现功能的 JavaScript -->
<script>
// 从隐藏的div中获取剧集信息和单集数据
const seriesInfo = JSON.parse(document.getElementById('series-info').textContent);
const episodesDataElement = document.getElementById('episodes-data');
const hasEpisodesData = episodesDataElement && episodesDataElement.textContent.trim() !== '[]';

// 处理单集数据：如果没有episodes-data，就基于endEpisode生成空数组
let episodesData;
let actualEndEpisode;

if (hasEpisodesData) {
  episodesData = JSON.parse(episodesDataElement.textContent);
  // 更新endEpisode为实际集数，确保一致性
  actualEndEpisode = seriesInfo.startEpisode + episodesData.length - 1;
} else {
  // 没有集名数据，基于endEpisode生成相应数量的空字符串
  const totalEpisodes = seriesInfo.endEpisode - seriesInfo.startEpisode + 1;
  episodesData = Array(totalEpisodes).fill("");
  actualEndEpisode = seriesInfo.endEpisode;
}

// 检查是否有任何集名（用于决定是否显示本集名列）
const hasAnyEpisodeName = hasEpisodesData && episodesData.some(name => name && name.trim() !== '');

// 您的 GitHub 信息
const githubUsername = 'LampingSu';
const repoName = 'lampingsu.github.io';
const releaseTag = '《佛陀传》有声书v0.1';

// 生成下载链接的函数
function getDownloadUrl(episodeNum) {
  const paddedEpisode = episodeNum.toString().padStart(seriesInfo.episodeDigits, '0');
  const filename = `${seriesInfo.enSeries}_${paddedEpisode}.${seriesInfo.ext}`;
  return `https://github.com/${githubUsername}/${repoName}/releases/download/${releaseTag}/${filename}`;
}

// 生成原始文件名
function getOriginalFilename(episodeNum, zhTitle) {
  const paddedEpisode = episodeNum.toString().padStart(seriesInfo.episodeDigits, '0');
  if (zhTitle) {
    return `${seriesInfo.zhSeries}_${paddedEpisode}_${zhTitle}.${seriesInfo.ext}`;
  } else {
    return `${seriesInfo.zhSeries}_${paddedEpisode}.${seriesInfo.ext}`;
  }
}

// 下载处理函数
function handleDownload(episodeNum, zhTitle) {
  const downloadUrl = getDownloadUrl(episodeNum);
  const originalFilename = getOriginalFilename(episodeNum, zhTitle);
  
  // 创建隐藏的链接并触发下载
  const link = document.createElement('a');
  link.href = downloadUrl;
  link.download = originalFilename;
  link.style.display = 'none';
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}

// 播放处理函数
function handlePlay(episodeNum, zhTitle, event) {
  const downloadUrl = getDownloadUrl(episodeNum);
  const originalFilename = getOriginalFilename(episodeNum, zhTitle);
  
  // 显示播放器容器
  const playerContainer = document.getElementById('audio-player-container');
  const nowPlayingTitle = document.getElementById('now-playing-title');
  const audioPlayer = document.getElementById('global-audio-player');
  
  // 更新播放器信息
  const displayTitle = zhTitle ? `${seriesInfo.zhSeries} 第${episodeNum.toString().padStart(seriesInfo.episodeDigits, '0')}集：${zhTitle}` 
                              : `${seriesInfo.zhSeries} 第${episodeNum.toString().padStart(seriesInfo.episodeDigits, '0')}集`;
  
  nowPlayingTitle.textContent = displayTitle;
  audioPlayer.src = downloadUrl;
  
  // 显示播放器并开始播放
  playerContainer.style.display = 'block';
  audioPlayer.style.display = 'block';
  
  // 尝试播放
  audioPlayer.play().catch(error => {
    console.log('自动播放被阻止，用户需要手动点击播放:', error);
  });
  
  // 阻止默认行为
  if (event) {
    event.preventDefault();
  }
}

// 构建表格HTML
function buildMediaTable() {
  // 动态构建表头
  let tableHtml = `
    <table class="media-table">
      <thead>
        <tr>
          <th>序号</th>
  `;
  
  // 只有在有集名的情况下才显示本集名列
  if (hasAnyEpisodeName) {
    tableHtml += `          <th>本集名</th>`;
  }
  
  tableHtml += `
          <th>操作</th>
        </tr>
      </thead>
      <tbody>
  `;

  // 构建表格内容
  episodesData.forEach((zhTitle, index) => {
    const episodeNum = seriesInfo.startEpisode + index;
    const downloadUrl = getDownloadUrl(episodeNum);
    const originalFilename = getOriginalFilename(episodeNum, zhTitle);
    
    tableHtml += `
      <tr>
        <td>${episodeNum.toString().padStart(seriesInfo.episodeDigits, '0')}</td>
    `;
    
    // 只有在有集名的情况下才显示本集名列
    if (hasAnyEpisodeName) {
      tableHtml += `        <td>${zhTitle || '-'}</td>`;
    }
    
    tableHtml += `
        <td>
          <button onclick="handleDownload(${episodeNum}, '${zhTitle}')" 
                  class="download-btn">下载</button>
          <button onclick="handlePlay(${episodeNum}, '${zhTitle}', event)" 
                  class="play-btn">播放</button>
        </td>
      </tr>
    `;
  });

  tableHtml += `
      </tbody>
    </table>
    
    <div style="margin-top: 10px; color: #666; font-size: 14px;">
      <strong>剧集信息：</strong>共 ${episodesData.length} 集（${seriesInfo.startEpisode} - ${actualEndEpisode}），文件格式为 .${seriesInfo.ext}
      ${!hasEpisodesData ? '<br><em>注：本剧集各集无单独名称</em>' : ''}
    </div>
  `;
  
  document.getElementById('media-list').innerHTML = tableHtml;
}

// 页面加载完成后生成表格
document.addEventListener('DOMContentLoaded', function() {
  buildMediaTable();
});
</script>